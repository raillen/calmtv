# User Documentation Map

## Learn the system
- [Getting started](tutorials/GETTING_STARTED.md)
- [Navigate with a remote](tutorials/NAVIGATE_WITH_REMOTE.md)

## Perform a task
- [Add IPTV](how-to/ADD_IPTV_PLAYLIST.md)
- [Add a ROM library](how-to/ADD_ROM_LIBRARY.md)
- [Organize downloads](how-to/ORGANIZE_DOWNLOADS.md)
- [Use network media](how-to/USE_NETWORK_MEDIA.md)
- [Use commercial streaming](how-to/USE_STREAMING_SERVICES.md)

## Exact reference
- [Controls](reference/CONTROLS.md)
- [Supported media/formats](reference/SUPPORTED_FORMATS.md)

## Understand behavior
- [Why apps close in the background](explanation/RESOURCE_MANAGEMENT.md)
- [Privacy and local AI](explanation/PRIVACY_AND_LOCAL_AI.md)
